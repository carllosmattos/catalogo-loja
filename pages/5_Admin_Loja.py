...
st.set_page_config(page_title="Admin — Loja", page_icon="🏪", layout="wide")
...
with st.form("store_form"):
    ...
    with col1:
        ...
        primary = st.color_picker(
            "Cor principal",
            value=settings.get("primary_color", "#E1306C"),
        )
        secondary = st.color_picker(
            "Cor secundária",
            value=settings.get("secondary_color", "#833AB4"),
        )
        accent = st.color_picker(
            "Cor de destaque",
            value=settings.get("accent_color", "#FCAF45"),
        )
...
preview_settings = fetch_store_settings()
